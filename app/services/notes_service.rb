class NotesService
  ...
  def self.fetch_note_by_id(user, note_id)
    cache_key = "user_#{user.id}_note_#{note_id}"
    Rails.logger.info "Checking Redis for key: #{cache_key}"
  ...
  def self.update_note(user, note_id, params)
    ...
    if note.update(params)
      ...
      $redis.del("user_#{user.id}_note_#{note_id}")  # Invalidate specific note cache
      $redis.setex("user_#{user.id}_note_#{note_id}", 600, note.to_json)  # Cache updated note
  ...
  def self.archive_note(user, note_id)
    ...
    $redis.del("user_#{user.id}_note_#{note_id}")  # Clear single note cache
